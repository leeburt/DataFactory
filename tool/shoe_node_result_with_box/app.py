...
def create_image_html(image_name, image_data):
    ...
    for component_coords, detail_info in sorted_component_items:
        ...
        # 生成该组件的连接可视化
        component_visualization = draw_single_component_with_connections(
            image_path, component_coords, {component_coords: detail_info}, component_details
        )
        
        # 创建连接可视化的HTML
        if component_visualization:
            visualization_html = f'''
            <div class="component-visualization">
                <img src="data:image/png;base64,{component_visualization}" alt="Component {component_name} connections" class="component-vis-image">
                <div class="vis-legend">
                    <div class="vis-legend-item">
                        <div class="vis-legend-color" style="background-color: red;"></div>
                        <span>当前组件</span>
                    </div>
                    <div class="vis-legend-item">
                        <div class="vis-legend-color" style="background-color: purple;"></div>
                        <span>输入连接区域</span>
                    </div>
                    <div class="vis-legend-item">
                        <div class="vis-legend-color" style="background-color: cyan;"></div>
                        <span>输出连接区域</span>
                    </div>
                    <div class="vis-legend-item">
                        <div class="vis-legend-color" style="background-color: orange;"></div>
                        <span>双向连接区域</span>
                    </div>
                    <div class="vis-legend-item">
                        <div class="vis-legend-color" style="background-color: purple;"></div>
            ...
            visualization_html = '<div class="component-visualization"><p>可视化生成失败</p></div>'
...
<head>
    ...
    <style>
        * {{
        ...
        .component-card.collapsed .component-content {{
            display: none;
        }}
...
def draw_single_component_with_connections(image_path, component_coords, component_details, all_component_details):
    """绘制单个组件及其连接的组件"""
    try:
        # 检查文件是否存在
        if not os.path.exists(image_path):
        ...
        # 当前组件的详细信息
        current_detail = component_details.get(component_coords, {})
        description = current_detail.get('description', {})
        
        # 解析当前组件坐标
        coords = eval(component_coords)
        x1, y1, x2, y2 = coords
        ...
        # 获取当前组件名称
        if isinstance(description, dict):
            component_name = description.get('component_name', '当前组件')
        else:
            component_name = '当前组件'
        ...
        name_x = x1
        name_y = max(0, y1 - 25)
        draw.text((name_x, name_y), component_name, fill='red', font=font)
        
        # 获取当前组件的输入输出端口
        det_io_info = current_detail.get('det_io_info', {})
        ...
        for input_port in input_ports:
            if len(input_port) == 4:
                ix1, iy1, ix2, iy2 = input_port
                draw.rectangle([ix1, iy1, ix2, iy2], outline='purple', width=3)
                draw.text((ix1, iy1-15), "IN", fill='purple', font=small_font)
        ...
        for output_port in output_ports:
            if len(output_port) == 4:
                ox1, oy1, ox2, oy2 = output_port
                draw.rectangle([ox1, oy1, ox2, oy2], outline='cyan', width=3)
                draw.text((ox1, oy1-15), "OUT", fill='cyan', font=small_font)
        
        # 坐标格式转换函数
        def convert_box_format(box_str):
            """将不同的坐标格式转换为标准的[x1, y1, x2, y2]格式"""
            if not box_str or box_str == "()":
                return None
            
            try:
                # 移除所有空格，然后处理不同的括号
                clean_str = box_str.replace(" ", "")
                
                # 如果是圆括号格式，转换为方括号
                if clean_str.startswith("(") and clean_str.endswith(")"):
                    # 去掉圆括号
                    coords_str = clean_str[1:-1]
                    # 解析坐标
                ...
                print(f"坐标格式转换失败: {box_str}, 错误: {e}")
                return None
        
        # 收集输入和输出连接的组件
        input_components = set()
        output_components = set()
        bidirectional_components = set()
        
        print(f"=== 开始处理组件连接: {component_coords} ===")
        print(f"当前组件描述: {description}")
        
        # 从描述中获取连接信息
        if isinstance(description, dict):
            connections = description.get('connections', {})
            print(f"连接信息: {connections}")
            
            # 处理输入连接
            for input_conn in connections.get('input', []):
                if isinstance(input_conn, dict) and 'box' in input_conn:
                    box_str = input_conn['box']
                    print(f"处理输入连接: {input_conn['name']}, box: {box_str}")
                    converted_box = convert_box_format(box_str)
                    if converted_box:
                        input_components.add(converted_box)
                        print(f"添加输入组件: {converted_box}")
            
            # 处理输出连接
            for output_conn in connections.get('output', []):
                if isinstance(output_conn, dict) and 'box' in output_conn:
                    box_str = output_conn['box']
                    print(f"处理输出连接: {output_conn['name']}, box: {box_str}")
                    converted_box = convert_box_format(box_str)
                    if converted_box:
                        output_components.add(converted_box)
                        print(f"添加输出组件: {converted_box}")
            
            # 处理双向连接
            for bidirectional_conn in connections.get('bidirectional', []):
                if isinstance(bidirectional_conn, dict) and 'box' in bidirectional_conn:
                    box_str = bidirectional_conn['box']
                    print(f"处理双向连接: {bidirectional_conn['name']}, box: {box_str}")
                    converted_box = convert_box_format(box_str)
                    if converted_box:
                        bidirectional_components.add(converted_box)
                        print(f"添加双向组件: {converted_box}")
        
        print(f"最终收集到的组件:")
        print(f"输入组件: {input_components}")
        print(f"输出组件: {output_components}")
        print(f"双向组件: {bidirectional_components}")
        print(f"所有组件详情的键: {list(all_component_details.keys())}")
        
        # 绘制输入连接的组件（紫色边框，与输入端口颜色一致）
        for conn_coords in input_components:
            try:
                conn_coords_list = eval(conn_coords)
                if len(conn_coords_list) == 4:
                    cx1, cy1, cx2, cy2 = conn_coords_list
                    # 直接绘制连接区域的框（紫色）
                    draw.rectangle([cx1, cy1, cx2, cy2], outline='purple', width=3)
                    
                    # 在连接区域上方写标签（紫色）
                    conn_name_x = cx1
                    conn_name_y = max(0, cy1 - 25)
                    draw.text((conn_name_x, conn_name_y), "输入连接", fill='purple', font=font)
                    
                    print(f"成功绘制输入连接区域: {conn_coords}")
                    
            except Exception as e:
                print(f"绘制输入连接区域 {conn_coords} 时出错: {e}")
                continue
        
        # 绘制输出连接的组件（青色边框，与输出端口颜色一致）
        for conn_coords in output_components:
            try:
                conn_coords_list = eval(conn_coords)
                if len(conn_coords_list) == 4:
                    cx1, cy1, cx2, cy2 = conn_coords_list
                    # 直接绘制连接区域的框（青色）
                    draw.rectangle([cx1, cy1, cx2, cy2], outline='cyan', width=3)
                    
                    # 在连接区域上方写标签（青色）
                    conn_name_x = cx1
                    conn_name_y = max(0, cy1 - 25)
                    draw.text((conn_name_x, conn_name_y), "输出连接", fill='cyan', font=font)
                    
                    print(f"成功绘制输出连接区域: {conn_coords}")
                    
            except Exception as e:
                print(f"绘制输出连接区域 {conn_coords} 时出错: {e}")
                continue
        
        # 绘制双向连接的组件（橙色边框）
        for conn_coords in bidirectional_components:
            try:
                conn_coords_list = eval(conn_coords)
                if len(conn_coords_list) == 4:
                    cx1, cy1, cx2, cy2 = conn_coords_list
                    ...
                    conn_name_x = cx1
                    conn_name_y = max(0, cy1 - 25)
                    draw.text((conn_name_x, conn_name_y), "双向连接", fill='orange', font=font)
                    
                    print(f"成功绘制双向连接区域: {conn_coords}")
                    
            except Exception as e:
                print(f"绘制双向连接区域 {conn_coords} 时出错: {e}")
                continue
